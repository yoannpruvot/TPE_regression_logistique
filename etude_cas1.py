...
chemin="C:/Users/yoann/Documents/cours/Mauffret_Pruvot_TPE/data1.csv"
#%%
...
def algorithme(x, y, theta):
    newton = fmin_tnc(func=fonction_cout, x0=theta,
...
#%%
nb_ind_tot=len(data)
nb_sport=np.histogram(data['sport'], bins=2)[0][1]
nb_non_sport=np.histogram(data['sport'], bins=2)[0][0]
print('La proportion totale de personne faisant du sport est de: {:.2f}%'.format(100*nb_sport/nb_ind_tot))
#%%
cat_vars=['sexe','nivetud','qualif','relig','trav.imp','trav.satisf']
for var in cat_vars:
    #cat_list='var'+'_'+var
...
X_col_names.remove('sport')
X=data[X_col_names].astype(int)
y=data['sport'].astype(int)
#%%
...
#%%
# On met en place X = (1,X) et on rajoute un dimension à y, et on prépare notre theta
X_train = np.c_[np.ones((X_train.shape[0], 1)), X]
